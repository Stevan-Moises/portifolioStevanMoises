...
function fecharModal() {
    modal.classList.remove('aberto'); document.body.style.overflow = 'auto';
}
...
// --- 4. Magnetismo Suave com Efeito Amortecido ---
...
cards.forEach(card => {
    card.addEventListener('mousemove', (e) => {
        // Bloqueia magnetismo se for touch
        ...
        // Mantemos a transição ativa do CSS (var(--transicao-magnetismo)) para suavizar a entrada
        card.style.transform = `rotateX(${rotateX}deg) rotateY(${rotateY}deg) scale(1.03)`;

        const pX = (x / rect.width) * 100;
        const pY = (y / rect.height) * 100;
        card.style.background = `radial-gradient(circle at ${pX}% ${pY}%, ${glowColor} 0%, ${glassColor} 50%)`;
    });

    card.addEventListener('mouseleave', () => {
        // Volta para o centro com uma transição de saída um pouco mais longa para elegância
        ...
        // Restaura a transição padrão após a animação de volta
        setTimeout(() => {
